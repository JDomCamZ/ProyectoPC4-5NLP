...
namespace Fusion.XR.Host
{
    ...
    public class ConnectionManager : MonoBehaviour, INetworkRunnerCallbacks
    {
        ...
        public NetworkObject userPrefab1;
        public NetworkObject userPrefab2;
        ...
        public void OnPlayerJoined(NetworkRunner runner, PlayerRef player)
        {
            ...
            if (runner.IsServer)
            {
                ...
                // Contar la cantidad de jugadores actualmente conectados
                int playerCount = runner.ActivePlayers.Count();

                // Seleccionar el prefab apropiado basado en la cantidad de jugadores
                NetworkObject selectedUserPrefab = playerCount == 1 ? userPrefab1 : userPrefab2;
                // We make sure to give the input authority to the connecting player for their user's object
                NetworkObject networkPlayerObject = runner.Spawn(selectedUserPrefab, position: transform.position, rotation: transform.rotation, inputAuthority: player, (runner, obj) => { 
                });

                var playerAvatar = networkPlayerObject.GetComponentInChildren<PlayerAvatar>();
                if (playerAvatar != null)
                {
                    playerAvatar.AvatarIndex = 0;
                }
